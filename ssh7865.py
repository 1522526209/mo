...
def send_kill_signal(remote_ip, remote_port=57866):
    """发送远程杀进程信号"""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((remote_ip, remote_port))
            s.send(b'KILL_PROCESS')
            print(f"已向{remote_ip}发送杀进程命令")
    ...
    def run(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                time.sleep(5)  # 等待 5 秒，模拟 5 秒内无错误发生
                print("端口转发请求成功。") # 添加成功提示
                break  # 成功请求端口转发后跳出循环
            except Exception as e:
                print(f'尝试 {attempt + 1}/{max_retries} 无法请求远程端口转发: {e}') # 显示尝试次数
                send_kill_signal(remote_ip)
